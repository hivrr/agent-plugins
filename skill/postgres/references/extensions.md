...
**Note:** For new projects, prefer `gen_random_uuid()` from pgcrypto or the built-in (PG13+) over uuid-ossp.
...
-- Area calculation
SELECT name, ST_Area(geom::geography) / 1000000 AS area_km2 FROM zones;

-- GeoJSON export
SELECT name, ST_AsGeoJSON(geom) AS geojson FROM locations;
```
...
SELECT pgp_sym_decrypt(encrypted_column, 'encryption-key') FROM table_name;
...
```bash
# Repack table — removes bloat and rebuilds indexes (run via command line, not SQL)
...
| Use Case | Extension |
|---|---|
| Query performance monitoring | `pg_stat_statements` (essential) |
| Fuzzy text matching / LIKE optimization | `pg_trgm` |
| Full-text search | Built-in (no extension needed) |
| Spatial / geographic data | `postgis` |
| Vector embeddings / AI / RAG | `pgvector` |
| Password hashing / encryption | `pgcrypto` |
| Time-series data | `timescaledb` |
| UUID generation (PG < 13) | `uuid-ossp` |
| Cross-database queries | `postgres_fdw` |
| Online bloat removal | `pg_repack` |
