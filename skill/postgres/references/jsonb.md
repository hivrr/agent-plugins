...
### GIN Index (Default — for containment and existence)
...
```sql
-- Index specific path for smaller index, faster queries on that path
...
```sql
-- Smaller (~20%), faster index for @> queries only
CREATE INDEX idx_documents_path_ops ON documents USING GIN(data jsonb_path_ops);

-- Good for:  WHERE data @> '{"key": "value"}'
-- Bad for:   WHERE data ? 'key' (not supported with this opclass)
```

### B-tree Index on Extracted Values

```sql
-- Index extracted scalar value — enables efficient equality and range queries
...
-- Array aggregation
SELECT jsonb_agg(data -> 'user') AS users
FROM documents
WHERE data @> '{"status": "active"}';

-- Object aggregation
SELECT jsonb_object_agg(id, data -> 'user') AS user_map
FROM documents
WHERE data ? 'user';
```
...
-- Extract frequently queried values to generated columns
...
WHERE data @> '{"score": "100"}'   -- Wrong: comparing string not number
WHERE CAST(data ->> 'score' AS INTEGER) = 100  -- Better (or use generated column)

-- Don't query without indexes on nested paths
SELECT * FROM documents WHERE data -> 'nested' -> 'deep' ->> 'value' = 'x';
-- Add: CREATE INDEX ON documents((data #>> '{nested,deep,value}'));
