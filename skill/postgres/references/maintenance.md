...
- Old rows are marked as "dead tuples"
...
```sql
-- Option 1: VACUUM FULL (locks table — avoid in production peak hours)
VACUUM FULL users;

-- Option 2: pg_repack (online, no locks — preferred for production)
...
-- Option 4: CLUSTER (rewrites table in index order, locks table)
...
-- Idle transactions (hold locks — bad)
SELECT
  pid,
  usename,
  state,
...
### Daily
- Monitor autovacuum activity (`pg_stat_user_tables.last_autovacuum`)
...
### Monthly
- Review and tune per-table autovacuum settings
- Reindex heavily updated indexes (`REINDEX INDEX CONCURRENTLY`)
- Update statistics on large tables (`ANALYZE`)
